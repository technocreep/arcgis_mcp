...
def make_tools(store: ProjectStore, state: dict) -> list[Callable]:
    ...
    def plot_layer(
        ...
        Args:
            ...
            style: Стиль рендеринга. "auto" — определяется автоматически:
                   точки ≤500 → "markers", 501–10 000 → "scatter",
                   >10 000 с числовым полем → "density" (линейная интерполяция griddata →
                   contourf градиент + изолинии contour с подписями clabel).
                   Для категориальных/без поля при >10 000 — scatter с мелкими маркерами.
                   Явные значения: "scatter" | "markers" | "density" | "lines" | "polygons".
                   Используй "density" для геофизических слоёв (магнитика, гравиметрия и т.п.).
