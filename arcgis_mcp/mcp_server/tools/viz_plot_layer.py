    ...
    def plot_layer(
        ...
        Args:
            ...
            style: Стиль рендеринга — выбирай явно по типу слоя:
                   ...
                   "scatter"  — точечный слой, среднее количество (500–10 000): мелкие точки.
                   "density"  — точечный слой, много объектов (>10 000) с числовым полем
                                (геофизика: магнитика, гравиметрия, геохимия и т.п.):
                                линейная интерполяция → заливка градиентом (contourf) +
                                изолинии (contour) с подписями значений (clabel).
