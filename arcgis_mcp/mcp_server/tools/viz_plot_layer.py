...
def make_tools(store: ProjectStore, state: dict) -> list[Callable]:
    ...
    def plot_layer(
        ...
        Args:
            ...
            style: Стиль рендеринга — выбирай явно по типу данных и геометрии:
                   "markers"  — дискретные точечные объекты, мало (≤500): скважины, пробы, точки наблюдений.
                   "scatter"  — дискретные точечные объекты, много (>500): скважины, шурфы, геохимические пробы.
                                Используй scatter для любого количества точек, если объекты дискретны
                                (каждая точка — отдельный объект, а не замер непрерывного поля).
                   "density"  — непрерывные физические поля: магнитика, гравиметрия, сейсмика, радиометрия.
                                Применяй когда точки — замеры непрерывной величины по профилям/сети,
                                обычно >1 000 точек. Интерполяция griddata → contourf + изолинии с подписями.
                   "lines"    — линейный слой (реки, дороги, профили, разломы).
                   "polygons" — полигональный слой (геология, лицензии, площади).
